...
command_test!("loop", {
    ...
    // for((i=0; i<3; i++)); do seq 1 10000; done | md5sum -t
    big_finite: [ sh "seq 1 10000 | %cmd% 3 | md5sum -t" assert "" => "5f0f7f173c062a0d128ff75ded51b09b  -\n" ],
    // for((i=0; i<10; i++)); do seq 1 10000; done | head -n40000 | md5sum -t
    big_infinite: [ sh "seq 1 10000 | %cmd% | head -n40000 | md5sum -t" assert "" => "3aeff2d35d8836dfbbdfe882848b5d30  -\n" ],
});
